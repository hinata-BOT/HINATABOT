...
//Modal dikit napa
//Kasi credit Ahmad Prasetyo 
//dan juga Subscribe ITSMEIKY OFFICIAL jgn cuma ganti nama bot
...
const apivhtear = 'b5389e4630e942baa526'
const BarBarKey = 'IDxO1TFYnKADlX4pxcHa'
const shizukaapi = 'itsmeiky633'
const TobzKey = 'BotWeA'
const vcard = 'BEGIN:VCARD\n'  // Jangan di ubah biar ga error
            + 'VERSION:3.0\n'  // Jangan di ubah biar ga error
            + 'FN:Owner NEKOBOT dan HINATA BOT\n'  // Ganti jadi namamu
            + 'ORG: Ahmad Prasetyo;\n'  // Ganti jadi namamu/Botmu
            + 'TEL;type=CELL;type=VOICE;waid=6287869984953:+6287869984953\n'  // Ganti jadi nomormu, tapi jangan ubah polanya
...
/******** OWNER NUMBER**********/
const ownerNumber = ["6287869984953@s.whatsapp.net","6281804503339"]  //ganti menjadi nomormulĺl
/************************************/
...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('qr already scan.subscribe','white'),color('YOU','red'),color('TUBE','white'),color('Ahmad - Kun','yellow'))
})
          ...
          const checkLimit = (sender) => {
           ...
           return false
			...
			switch(command) {
                ...
                addKoinUser('6287869984953@s.whatsapp.net', fee)
			...
			if (args.length < 1) return reply(ind.wrongf())
                 ...
                 if (isLimit(sender)) return reply(ind.limitend(pusname))
                   ...
                   const aruga1 = q.substring(0, q.indexOf('|') - 0)
					...
					buffer = await getBuffer(`https://api.vhtear.com/silktext?text=${silk}&apikey=${b5389e4630e942baa526}`)
					reply(ind.wait())
					...
					teks = `${body.slice(7)}`
					atytyd = await getBuffer(`https://api.vhtear.com/slidingtext?text=${teks}&apikey=${b5389e4630e942baa526}`, {method: 'get'})
					reply(ind.wait)
					...
					cml1 = cml.split("/")[0];
					cml2 = cml.split("/")[1];
					buffer = await getBuffer(`https://api.vhtear.com/logoml?hero=${cml1}&text=${cml2}&apikey=${b5389e4630e942baa526}`, {method: 'get'})
					...
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                    			...
                    			text1 = hm.split("/")[0];
                    			text2 = hm.split("/")[1];                    
                    			glitch = await getBuffer(`https://api.vhtear.com/glitchtext?text1=${text1}&text2=${text2}&apikey=${b5389e4630e942baa526}`, {method: 'get'})
...
har = body.slice(12)
buffer = await getBuffer(`https://api.vhtear.com/hartatahta?text=${har}&apikey=${b5389e4630e942baa526}` )
...
break
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
                    ...
                    buffer6 = await getBuffer(`https://api.vhtear.com/padlock?text1=${teks}&text2=${teks}&apikey=${b5389e4630e942baa526}`, {method: 'get'})
		...
		break
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					pinte = body.slice(11)
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=${pinte}&apikey=${b5389e4630e942baa526}`, {method: 'get'})
					reply(ind.wait())
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=loli&apikey=${b5389e4630e942baa526}`, {method: 'get'})
					reply(ind.wait())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					anu = await fetchJson(`https://api.vhtear.com/randomloli&apikey=${b5389e4630e942baa526}`, {method: 'get'})
					reply(ind.wait())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					anu = await fetchJson(`https://api.vhtear.com/randomnekonime&apikey=${b5389e4630e942baa526}`, {method: 'get'})
					reply(ind.wait())
					buffer = await getBuffer(anu.result.result)
				...
				if (!isNsfw) return reply(ind.nsfwoff())
				if (!isGroup) return reply(ind.groupo()) 
				res = await fetchJson(`https://api.vhtear.com/randomloli&apikey=${b5389e4630e942baa526}`, {method: 'get'})
					reply(ind.wait())
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					data = await fetchJson(`https://api.vhtear.com/randomwibu&apikey=${b5389e4630e942baa526}`)
					reply(ind.wait())
			...
			if (!isGroup) return reply(ind.groupo()) 
              	    if (args.length < 1) return reply('teksnya mana gan?')
                    teks = body.slice(9)
                    anu = await fetchJson(`https://api.vhtear.com/nekosearch?query=${teks}&apikey=${b5389e4630e942baa526}`, {method: 'get'})
			reply(ind.wait())
                    teks = `===============\n`
		...
		case 'resepmasakan':  
                ...
                if (isLimit(sender)) return reply(ind.limitend(pushname))
                reply(ind.wait)
                   anu = await fetchJson(`https://api.vhtear.com/resepmasakan?query=${body.slice(12)}&apikey=${b5389e4630e942baa526}`, {method: 'get'})
                   ...
                   if (isLimit(sender)) return reply(ind.limitend(pusname))
                     teks = body.slice(9)
                     anu = await fetchJson(`https://api.vhtear.com/igprofile?query=${teks}&apikey=${b5389e4630e942baa526}`, {method: 'get'})
			...
			if (!isGroupAdmins) return reply(ind.admin())
					...
					if (!isRegistered) return reply(ind.noregis())
					client.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Ini adalah list group NEKO BOT dan HINATA BOT :\n\n\`\`\``
					no = 0
	...
	if (!isRegistered) return reply(ind.noregis())
           if (isLimit(sender)) return reply(ind.limitend(pusname))
			...
			if (!isRegistered) return reply(ind.noregis())
				...
				sa = pa.split("/")[0];
				ngan = pa.split("/")[1];
				anu = await fetchJson(`https://api.vhtear.com/primbonjodoh?nama=${sa}&pasangan=${ngan}&apikey=${b5389e4630e942baa526}`, {method: 'get'})
	  ...
	  case 'seberapagay':
		...
		if (isLimit(sender)) return reply(ind.limitend(pusname))				
		if (!isGroup) return reply(ind.groupo())                 
				if (args.length < 1) return reply(`Gini bego: ${prefix}fitnah [@tag&pesan&balasanbot]\n\nContoh : ${prefix}fitnah @tagmember&hai&hai juga`)
				var gh = body.slice(8)
				mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					...
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pushname))
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query={body.slice(11)}&apikey=${b5389e4630e942baa526}`, {method: 'get'})
	...
	case 'hobby':
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
					hobby = body.slice(1)
					const hob =['Desah Di Game','Ngocokin Doi','Stalking sosmed nya mantan','Kau kan gak punya hobby awokawok','Memasak','Membantu Atok','Mabar','Nobar','Sosmedtan','Membantu Orang lain','Nonton Anime','Nonton Drakor','Naik Motor','Nyanyi','Menari','Bertumbuk','Menggambar','Foto fotoan Ga jelas','Maen Game','Berbicara Sendiri','Nonton Wibu','Kau kan hobby ngewe']
		...
		if (isLimit(sender)) return reply(ind.limitend(pusname))
		if (!isGroup) return reply(ind.groupo())
					anu = await fetchJson(`https://api.vhtear.com/funkuis&apikey=${b5389e4630e942baa526}`, {method: 'get'})
		...
		if (isLimit(sender)) return reply(ind.limitend(pusname))
		if (!isGroup) return reply(ind.groupo())
					anu = await fetchJson(`https://api.vhtear.com/family100&apikey=${b5389e4630e942baa526}`, {method: 'get'})
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://api.vhtear.com/tebakgambar&apikey=${b5389e4630e942baa526}`, {method: 'get'})
		...
		reply(ind.wait)
    		...
    		if (!isRegistered) return reply(ind.noregis())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					anu = await fetchJson(`https://api.vhtear.com/ytdl?link=${args[0]}&apikey=${b5389e4630e942baa526}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
		...
		if (isLimit(sender)) return reply(ind.limitend(pusname)) 
                reply(ind.wait())
                anu = await fetchJson(`https://api.vhtear.com/ytmp3?query=${body.slice(6)}&apikey=${b5389e4630e942baa526}`)
               if (anu.error) return reply(anu.error)
...
case 'tarekses':
case 'tarikses':
const tariksismp3 = fs.readFileSync('./src/assets/tariksis.mp3');
client.sendMessage(from, tariksismp3, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
...
break
case 'sound3':
tiga = fs.readFileSync('./src/assets/sound3.m4a')
...
break
         case 'admin':
            ...
            case 'creator':
                  ...
                  client.sendMessage(from, 'Tuh nomer owner ku >_<, namanya ahmad save yah lagi butkon',MessageType.text, { quoted: mek} )
					break    
           	case 'setname':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
